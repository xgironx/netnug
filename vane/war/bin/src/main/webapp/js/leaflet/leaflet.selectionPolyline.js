(function(){
	...
	.factory("SelectionPolyline", function(leaflet){
		return leaflet.Polyline.extend({
			...
			_updateWeight: function(){
				this.setStyle({weight: this._link.width() ? this._link.width() + 4 : 10});
			},
			...
			// SelectionPolyline uses it's parent's polyline curve, so it doesn't need its own projection
			_project: function(){},
